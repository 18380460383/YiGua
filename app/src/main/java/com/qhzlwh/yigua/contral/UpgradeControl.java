package com.qhzlwh.yigua.contral;
...
import com.qhzlwh.yigua.bean.UpdateMsg;
...
public class UpgradeControl {
    ...
    private UpgradeControl(Context context){
        if(context==null){
            context= MyApplication.getInstance().getOneActivity();
        }else{
...
/*        String channel = AppContext.getInstance().getChannel();
        ...
        OkHttpUtils.get().url(Myconstant.getAppBate+"2").build().execute(new StringCallback() {
            @Override
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    Gson gson=new Gson();
                    UpdateMsg data=gson.fromJson(response,UpdateMsg.class);
                    //   final UpdateMsg data = JsonUtils.getBean(jsonObject.optJSONObject("data"), UpdateMsg.class);
                    System.out.println("升级哈哈" + data);
                    if (MyUtil.getVersionCode(context) < Integer.valueOf(data.getData().getBate())) {
                        showUpdataDialog(jsonObject, data.getData());
                    } else {
                       // showUpdataDialog(jsonObject, data);
                       // showMsg("","版本号："+data.getData().getBate()+"\n版本名："+data.getData().getShowversion()+"\n应用名:"+data.getData().getQzcont()+"\n描述："+data.getData().getNot_version_alert());
                        LogUtil.e("update","版本为最新版本");
                        MyApplication.getInstance().setDownId((long) 0);
                    }
 ...
 /*       NetworkDownload.byteGet(context, Myconstant.BASE_OUT_URL, params, new NetworkDownload.NetworkDownloadCallBackbyte() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] bytes) {
                ...
                try {
                    ...
                    UpdateMsg data=gson.fromJson(json,UpdateMsg.class);
                 //   final UpdateMsg data = JsonUtils.getBean(jsonObject.optJSONObject("data"), UpdateMsg.class);
                    System.out.println("升级哈哈" + data);

                    if (MyUtil.getVersionCode(context) < Integer.valueOf(data.getBate())) {
                        showUpdataDialog(jsonObject, data);
                    } else {
     ...
     * @param data
     */
    private void showUpdataDialog(JSONObject body, final UpdateMsg.DataBean data) {
        ...
        yesUpdate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
               /* Uri  uri = Uri.parse(data.getDownload());
                ...
                Long downId = MyApplication.getInstance().getDownId();
                if (downId != 0) {
    ...
    private void queryDownloadStatus(final DownloadManager dowanloadmanager, final Long lastDownloadId, final UpdateMsg.DataBean data) {
    ...
    private void down(DownloadManager downloadManager, UpdateMsg.DataBean data) {
        Uri uri = Uri.parse((String) data.getDownload());
        //Uri uri = Uri.parse("http://pre.im/4fed");
        DownloadManager.Request request = new DownloadManager.Request(uri);
        request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED);
        File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath() + "/yigua/");
        if(!file.exists()){
            file.mkdirs();
        }
        File filee=new File(file.getAbsolutePath()+"yigua" + data.getShowversion() + ".apk");
        ...
        MyApplication.getInstance().setDownId(reference);
    }
    ...
    class DownloadChangeObserver extends ContentObserver {
        ...
        public void onChange(boolean selfChange) {
            if (null == query) {
                ...
                query.setFilterById(MyApplication.getInstance().getDownId());
            }
            ...
            if (c != null && c.moveToFirst()) {
                ...
                if(!selfChange&&bytesDL==fileSize){
                    pbarDialog.dismiss();
                    Uri uriForDownloadedFile = ((DownloadManager) context.getSystemService(Context.DOWNLOAD_SERVICE)).getUriForDownloadedFile(MyApplication.getInstance().getDownId());
                    Intent intent = new Intent(Intent.ACTION_VIEW);
                    intent.setDataAndType(uriForDownloadedFile,
                            "application/vnd.android.package-archive");
                    context.startActivity(intent);
