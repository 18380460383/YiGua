	...
	public void onRegeocodeSearched(RegeocodeResult regeocodeReult,int resultCode) {
		if (resultCode == 0) {
			if (regeocodeReult != null
				...
				RegeocodeAddress regeocodeAddress = regeocodeReult.getRegeocodeAddress();
                String formatAddress = regeocodeAddress.getFormatAddress();
                String shortAdd = formatAddress.replace(regeocodeAddress.getProvince(), "").replace(regeocodeAddress.getCity(), "").replace(regeocodeAddress.getDistrict(), "").replace(regeocodeAddress.getTownship(), "");
				PositionEntity entity = new PositionEntity();
				entity.address = shortAdd;
				entity.city = city;
